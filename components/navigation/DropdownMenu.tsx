...
export const TutorDropdownMenu = () => {
  const axiosPrivate = useTutorAxiosPrivate();
  const { auth } = useAuth();
  const email = auth.email ?? "";
  const [tutor, setTutor] = useState<{ name?: string | null; imagePath?: string | null } | null>(null);
  const [profileImageError, setProfileImageError] = useState(false);
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          <Image
            src={profileImageError ? DPDefault : (tutor?.imagePath || DPDefault)}
            alt={tutor?.name || "Tutor"}
            width={36}
            height={36}
            className="rounded-full object-cover"
            onError={() => setProfileImageError(true)}
            unoptimized={!!(tutor?.imagePath && !tutor.imagePath.startsWith("/"))}
          />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-56 bg-blue-900 text-white border-blue-700">
        ...
        <DropdownMenuSeparator className="bg-blue-700" />
        <DropdownMenuItem asChild className="hover:bg-blue-800 focus:bg-blue-800 text-white cursor-pointer">
          <Link href="/tutor" className="flex items-center space-x-2 w-full">
            <FontAwesomeIcon icon={faChalkboardTeacher} className="h-4 w-4" />
            <span>Dashboard</span>
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild className="hover:bg-blue-800 focus:bg-blue-800 text-white cursor-pointer">
          <Link href="/tutor/settings" className="flex items-center space-x-2 w-full">
            <FontAwesomeIcon icon={faGear} className="h-4 w-4" />
            <span>Settings</span>
          </Link>
        </DropdownMenuItem>
